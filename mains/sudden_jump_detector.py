...
def get_sudden_jump(high_low_ratio_dict, threshold = 2):
    sudden_jumps = []
    for interval in high_low_ratio_dict:
        if high_low_ratio_dict[interval] > threshold:
...
sudden_jump_dict = get_suddent_jump_results(get_cryptocurrency_list_bittrex()[:10], _threshold=1.5)
